...
def move_player(player, dir, upper_bound):
    ...
    update the user’s location by changing the row and column values in the location
    list by adding or subtracting 1 to the row or column (depending on the direction
    they moved), otherwise, display an error message and do not update the user’s
    location.
    """
    pass
...
def count_treasures(map, player, upper_bound):
    """
    iterates through
    the surrounding spaces of the user's current location. Keep a count of the number of
    treasures (T'), and traps ('X') that are in those spaces. Return the two counts.
    """
    pass
